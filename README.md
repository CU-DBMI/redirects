...
This is where you specify where you want to redirect from and to.
...
1. You change one or more of the `.yaml` files in the top folder of the _redirects repo_.
...
3. The `encode.js` script combines all of your top-folder `.yaml` files into one, and encodes it into a format that isn't searchable or human-readable[^1].
...
[^1]: This approach does not _encrypt_ your redirect list, it only _obfuscates_ it. Anyone with some coding knowledge could still figure out all of your redirect lists with some effort. The way this method works, true encryption is impossible because any key needed for decryption would also need to be in the `redirect.js` script, which the attacker would have access to.
